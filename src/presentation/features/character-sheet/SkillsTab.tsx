...
export function SkillsTab({ character, onChange }: SkillsTabProps) {
  ...
  const totals = deriveSkillTotals(
    character,
    CLASSES_BY_ID,
    SKILLS_BY_ID,
    RACES_BY_ID,
  ...
  const effectiveAbilityScores = deriveEffectiveAbilityScores(character, RACES_BY_ID);
  const budget = deriveSkillPointBudget(
    character,
    CLASSES_BY_ID,
    RACES_BY_ID,
  ...
  return (
    <div>
      <p
        className={`mb-2 text-sm font-medium ${budget.spent > budget.available ? "text-destructive" : "text-muted-foreground"}`}
      >
        {t("characterSheet.skills.pointsBudget", {
          spent: budget.spent,
                ...
                <tr key={skillDef.id} className="border-b border-border/60">
                  ...
                  <td className="px-2 py-1.5">
                    <NumberInput
                      ...
                      onChange={(e) =>
                        updateSkill(skillDef.id, { miscModifier: Number(e.target.value) })
                      }
                      className="!w-16 text-center"
                    />
                  </td>
