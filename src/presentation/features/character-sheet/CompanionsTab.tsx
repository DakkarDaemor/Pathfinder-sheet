...
import {
  deriveCompanionCombatSheet,
  resolveCompanionBaseStats,
} from "@/domain/companion/calculations";
...
import { feetToMeters, metersToFeet, roundMetric } from "@/shared/units";
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4">
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        />
        <Select
          value={companion.baseId ?? CUSTOM_BASE_OPTION}
      ...
      {customBase ? (
        <div className="mb-3 flex flex-col gap-2 rounded-md border border-border p-3">
          <Field label={t("characterSheet.companions.customName")}>
            <TextInput
              value={customBase.name}
              onChange={(e) => updateCustomBase({ name: e.target.value })}
            />
          </Field>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            <Field label={t("characterSheet.profile.size")}>
              <Select
                ...
                onChange={(e) =>
                  updateCustomBase({ size: e.target.value as CompanionCustomBase["size"] })
                }
              >
            ...
            <Field
              label={`${t("characterSheet.companions.speed")} (${t("characterSheet.combat.meters")})`}
            >
              <NumberInput
                value={roundMetric(feetToMeters(customBase.speed))}
                onChange={(e) => updateCustomBase({ speed: metersToFeet(Number(e.target.value)) })}
              />
            </Field>
          </div>
      ...
      <div className="mb-3 grid grid-cols-2 gap-2 text-center sm:grid-cols-4">
        <div className="rounded-md border border-border p-2">
          <div className="text-[10px] text-muted-foreground">
            {t("characterSheet.combat.acNormal")}
          </div>
          <div className="text-sm font-semibold">{sheet.armorClass.normal}</div>
        </div>
        <div className="rounded-md border border-border p-2">
          <div className="text-[10px] text-muted-foreground">
            {t("characterSheet.combat.baseAttackBonus")}
          </div>
          <div className="text-sm font-semibold">+{sheet.baseAttackBonus}</div>
        </div>
        <div className="rounded-md border border-border p-2">
          <div className="text-[10px] text-muted-foreground">
            {t("characterSheet.combat.cmb")}/{t("characterSheet.combat.cmd")}
          </div>
        ...
        <div className="rounded-md border border-border p-2">
          <div className="text-[10px] text-muted-foreground">
            {t("characterSheet.combat.fort")}/{t("characterSheet.combat.ref")}/
            {t("characterSheet.combat.will")}
          </div>
          <div className="text-sm font-semibold">
        ...
        <div className="mb-4 flex flex-col gap-4">
          {linkedCompanions.map((companion) => (
            <CompanionCard
              ...
              master={master}
              onUpdate={(patch) =>
                updateCompanionInStore(companion.id, (c) => ({ ...c, ...patch }))
              }
